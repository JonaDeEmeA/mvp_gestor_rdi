...
export const mapBCFTopicToRDI = (topic, viewpointsSistema, viewpointsArchivo = null) => {
  ...
  return {
    ...
    status: topic.status || 'Abierta',
    label: topic.labels && topic.labels.size > 0 ? Array.from(topic.labels)[0] : 'General',
    assignedTo: topic.assignedTo || 'coordinacion@gmail.com',
    dueDate: topic.dueDate ? new Date(topic.dueDate).toISOString() : null,
